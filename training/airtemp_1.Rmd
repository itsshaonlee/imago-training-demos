...
temp_csv <- read_csv("./data/temperature_indicators_MSOA_2024.csv")
glimpse(temp_csv)

# Load the MSOA boundaries
msoa_bounds <- read_sf("./data/UK_MSOA_IZB_SDZ/UK_MSOA_IZB_SDZ.gpkg")
glimpse(msoa_bounds)


# Merge the two
msoa_temp <- msoa_bounds %>%
  left_join(temp_csv, by = c("data_zone_code" = "dt_zn_c"))

# Ensure it is a spatial data frame  
msoa_temp <- st_as_sf(msoa_temp, sf_column_name = "geom")
# should now show "sf"
glimpse(msoa_temp)
...
To ease working with the lookup, a smaller file containing just the relevant columns is provided in the `data` folder. You can also download the full file from ONS and use it, but be aware that it is quite large (over 1GB) and may crash this Binder session.
With provided file, you can load it directly like so:
```{r}
LSOA21_MSOA21 <- read_csv("./data/lsoa_msoa_lookup.csv") %>%
...
```{r}
msoa_temp <- msoa_temp %>%
  mutate(msoa21cd = trimws(as.character(data_zone_code)))
